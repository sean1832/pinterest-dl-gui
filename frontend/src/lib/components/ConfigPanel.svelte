<script lang="ts">
	...
	const captionLabel = $derived(captionItems.find((i) => i.value === run.caption)?.label ?? "Select");
	const clientLabel = $derived(clientItems.find((i) => i.value === run.client)?.label ?? "Select");
</script>

{#snippet groupLabel(text: string)}
	<span class="text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">{text}</span>
{/snippet}
...
<section class="flex min-h-0 shrink-0 flex-col bg-background" style="width: {width}px">
	<ScrollArea class="min-h-0 flex-1">
		<div class="flex flex-col gap-6 p-6">
			<!-- Mode -->
			<ToggleGroup type="single" bind:value={run.mode} class="w-full rounded-md border border-border bg-card p-1">
				<ToggleGroupItem
					value="scrape"
				...
				<div class="flex flex-col gap-1.5">
					...
					<div class="flex">
						<Input
							...
							class={cn("flex-1 font-mono", showBrowseSource && "rounded-r-none border-r-0")}
						/>
						{#if showBrowseSource}
				...
				<div class="flex gap-3">
					...
					{#if showLimit}
						<div class="flex flex-1 flex-col gap-1.5">
							...
							<Input id="limit" type="number" bind:value={run.limit} min="1" max="5000" />
						</div>
					{/if}
				</div>
			</div>
			...
			<div class="flex flex-col gap-3">
				...
				<div class="flex flex-col gap-2">
					<OptionRow title="Fetch Videos" desc="Download HLS video segments and mux to MP4.">
					...
					>
						<div class="flex items-center gap-2">
							<Input type="number" bind:value={run.resW} min="0" class="w-16 font-mono" />
							<span class="text-muted-foreground">x</span>
							<Input type="number" bind:value={run.resH} min="0" class="w-16 font-mono" />
						</div>
					</OptionRow>

					<OptionRow title="Metadata Strategy" desc="Format for accompanying alt text/captions.">
			...
			<div class="flex flex-col gap-3">
				...
				<Accordion.Root type="multiple" class="flex flex-col gap-2">
					<Accordion.Item value="engine" class="rounded-md border border-border bg-card px-3">
						...
						<Accordion.Content class="flex flex-col gap-3 border-t border-border pt-3">
							<div class="flex flex-col gap-1.5">
								...
								<Select.Root type="single" bind:value={run.client} disabled={clientDisabled}>
							...
							{#if showIncognito}
								<OptionRow title="Incognito Context" desc="Isolate browser session state.">
